...
Think of this file as the agent's capability allowlist: which actions the model
may request, how those actions validate their arguments, and how they are
ultimately executed are all defined here.
"""
...
def build_tool_registry(context):
    # Tools are not discovered dynamically; they are registered explicitly. This
    # way the model sees a bounded, auditable set of actions.
    tools = {
        name: {**spec, "run": partial(_TOOL_RUNNERS[name], context)}
        for name, spec in BASE_TOOL_SPECS.items()
    }
    # The sub-agent is deliberately limited in capability: once the depth budget
    # is exhausted, even the delegate tool is no longer exposed to the model.
...
def validate_tool(context, name, args):
    ...
    if name == "patch_file":
        # patch_file is deliberately strict: old_text must match exactly and
        # occur only once, so the edit is deterministic and failures are easier
        # to explain.
        path = context.path(args["path"])
        if not path.is_file():
            raise ValueError("path is not a file")
...
def tool_search(context, args):
    ...
    if shutil.which("rg"):
        # Prefer rg, because searching happens very frequently and search
        # latency directly affects the agent's control loop.
...
def tool_run_shell(context, args):
    ...
    result = subprocess.run(
        ...
        text=True,
        timeout=timeout,
        # We pass a filtered set of environment variables here instead of
        # inheriting the entire parent shell environment, to reduce the risk of
        # sensitive information accidentally leaking into the command's
