...
from keel.evaluation.metrics import (  # noqa: E402
    collect_project_metrics,
    render_large_scale_experiment_report,
    render_project_metrics_markdown,
    run_provider_experiments,
)


def build_arg_parser():
    parser = argparse.ArgumentParser(description="Run keel large-scale experiments and write all experiment artifacts.")
    parser.add_argument("--benchmark-artifact", required=True, help="Path to benchmark artifact JSON.")
    parser.add_argument("--runs-root", required=True, help="Path to .keel/runs root.")
    ...
    parser.add_argument("--metrics-output-json", required=True)
    parser.add_argument("--metrics-output-markdown", required=True)
...
def main(argv=None):
    ...
    outputs = {
        args.metrics_output_json: metrics,
    ...
    metrics_md = Path(args.metrics_output_markdown)
    metrics_md.parent.mkdir(parents=True, exist_ok=True)
    metrics_md.write_text(render_project_metrics_markdown(metrics) + "\n", encoding="utf-8")
