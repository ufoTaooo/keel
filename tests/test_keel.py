...
def build_agent(tmp_path, outputs, **kwargs):
    ...
    store = SessionStore(tmp_path / ".keel" / "sessions")
...
def test_list_files_hides_internal_agent_state(tmp_path):
    agent = build_agent(tmp_path, [])
    (tmp_path / ".keel").mkdir(exist_ok=True)
    (tmp_path / ".git").mkdir(exist_ok=True)
    (tmp_path / "hello.txt").write_text("hi\n", encoding="utf-8")

    result = agent.run_tool("list_files", {})

    assert ".keel" not in result
...
def test_welcome_screen_keeps_box_shape_for_long_paths(tmp_path):
    deep = tmp_path / "very" / "long" / "path" / "for" / "the" / "keel" / "agent" / "welcome" / "screen"
...
def test_openai_compatible_client_posts_expected_responses_payload():
    ...
    assert captured["headers"]["User-agent"] == "keel/0.1"
...
def test_build_agent_uses_openai_provider_and_model_override(tmp_path):
    ...
    with patch.dict(
        ...
        with patch(
            "keel.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("keel.cli.OpenAICompatibleModelClient") as mock_openai:
            fake_client = mock_openai.return_value
            agent = keel_pkg.build_agent(args)
...
def test_build_agent_uses_right_codes_shared_key_for_openai_provider(tmp_path):
    ...
    with patch.dict(os.environ, {"KEEL_RIGHT_CODES_API_KEY": "sk-right-codes"}, clear=True):
        with patch(
            "keel.cli.OllamaModelClient",
            side_effect=AssertionError("ollama client should not be used"),
        ), patch("keel.cli.OpenAICompatibleModelClient") as mock_openai:
...
def test_build_arg_parser_leaves_provider_unset_for_runtime_resolution(tmp_path):
    args = keel_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path)])

    assert args.provider is None


def test_build_arg_parser_accepts_anthropic_provider(tmp_path):
    args = keel_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "anthropic"])

    assert args.provider == "anthropic"


def test_build_arg_parser_accepts_deepseek_provider(tmp_path):
    args = keel_pkg.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
...
def test_build_agent_uses_project_env_provider_when_cli_omitted(tmp_path):
    (tmp_path / ".env").write_text(
        "\n".join(
            [
                ...
                "KEEL_OPENAI_API_KEY=sk-project-openai",
                "KEEL_OPENAI_MODEL=gpt-5.4",
                "KEEL_DEEPSEEK_API_KEY=sk-project-deepseek",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "keel.cli.AnthropicCompatibleModelClient",
...
def test_build_agent_prefers_cli_provider_over_project_env_provider(tmp_path):
    ...
    args = keel_pkg.build_arg_parser().parse_args(
    ...
    with patch.dict(os.environ, {}, clear=True):
        with patch(
            ...
            "keel.cli.OpenAICompatibleModelClient",
            side_effect=AssertionError("openai client should not be used"),
        ), patch("keel.cli.AnthropicCompatibleModelClient") as mock_anthropic:
...
def test_build_agent_uses_anthropic_default_model_when_env_is_missing(tmp_path):
    ...
    with patch.dict(
        ...
        with patch("keel.cli.AnthropicCompatibleModelClient") as mock_anthropic:
            keel_pkg.build_agent(args)
...
def test_explicit_memory_promotion_persists_durable_memory_topics(tmp_path):
    agent = build_agent(
        ...
        [
            ...
            "Project convention: Preserve local agent state under .keel/.\n"
    ...
    index_path = tmp_path / ".keel" / "memory" / "MEMORY.md"
    conventions_path = tmp_path / ".keel" / "memory" / "topics" / "project-conventions.md"
    decisions_path = tmp_path / ".keel" / "memory" / "topics" / "key-decisions.md"
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    ...
    assert report["durable_promotions"] == [
        ...
        "project-conventions: Preserve local agent state under .keel/.",
...
def test_explicit_memory_promotion_rejects_secret_shaped_and_transient_lines(tmp_path):
    ...
    report = json.loads(agent.run_store.report_path(agent.current_task_state).read_text(encoding="utf-8"))
    conventions_path = tmp_path / ".keel" / "memory" / "topics" / "project-conventions.md"
    dependency_path = tmp_path / ".keel" / "memory" / "topics" / "dependency-facts.md"
...
def test_public_api_exports_resolve_through_package_path():
    ...
    assert Path(keel_pkg.__file__).as_posix().endswith("/keel/__init__.py")


def test_package_import_surface_includes_cli_entrypoints():
    assert callable(keel_pkg.main)
    assert callable(keel_pkg.build_agent)
    assert callable(keel_pkg.build_arg_parser)


def test_module_execution_help_works():
    result = subprocess.run(
        [sys.executable, "-m", "keel", "--help"],
        capture_output=True,
        text=True,
    )
