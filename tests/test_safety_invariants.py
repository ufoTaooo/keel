...
from keel import FakeModelClient, Keel, SessionStore, WorkspaceContext
from keel import cli as keel_cli
from keel.task_state import TaskState
...
def test_cli_build_agent_wires_secret_env_names_from_parser(tmp_path):
    ...
    with patch.dict(os.environ, {"GITHUB_PAT": "ghp-1", "GH_PAT": "ghp-2"}, clear=True), patch(
        ...
        args = keel_cli.build_arg_parser().parse_args(
            [
                "--cwd",
                str(tmp_path),
                "--approval",
        ...
        agent = keel_cli.build_agent(args)
...
def test_cli_build_agent_uses_default_configured_secret_names(tmp_path):
    ...
    with patch.dict(os.environ, {"GH_PAT": "ghp-default-1"}, clear=True), patch(
        ...
        args = keel_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = keel_cli.build_agent(args)
...
def test_cli_build_agent_loads_project_env_secrets_before_redaction_setup(tmp_path):
    ...
    (tmp_path / "README.md").write_text("demo\n", encoding="utf-8")
    (tmp_path / ".env").write_text("KEEL_DEEPSEEK_API_KEY=sk-project-secret\n", encoding="utf-8")
    with patch.dict(os.environ, {}, clear=True), patch("keel.cli.AnthropicCompatibleModelClient", DummyModelClient):
        args = keel_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--provider", "deepseek"])
        agent = keel_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["KEEL_DEEPSEEK_API_KEY"]
...
def test_cli_build_agent_reads_secret_names_from_environment_config(tmp_path):
    ...
    with patch.dict(
        os.environ,
        {
            "KEEL_CUSTOM_SECRET": "custom-secret-value",
            "KEEL_SECRET_ENV_NAMES": "KEEL_CUSTOM_SECRET",
        },
        clear=True,
    ), patch("keel.cli.OllamaModelClient", DummyModelClient):
        args = keel_cli.build_arg_parser().parse_args(["--cwd", str(tmp_path), "--approval", "auto"])
        agent = keel_cli.build_agent(args)
        assert agent.secret_env_summary()["secret_env_names"] == ["KEEL_CUSTOM_SECRET"]


def test_run_shell_uses_allowlisted_environment_only(tmp_path):
    secret = "shh-allowlist-secret"
    agent = build_agent(tmp_path, [], approval_policy="auto")
    script = 'import os; print(os.getenv("KEEL_ALLOWLIST_SECRET", "missing"))'
    command = f"{shlex.quote(sys.executable)} -c {shlex.quote(script)}"

    with patch.dict(os.environ, {"KEEL_ALLOWLIST_SECRET": secret}, clear=False):
...
def test_bound_tool_methods_delegate_into_tools_module(tmp_path):
    ...
    with patch("keel.tools.subprocess.run") as fake_run:
    ...
    assert agent.tool_run_shell.__func__.__module__ == "keel.runtime"

    with patch("keel.tools.tool_delegate", return_value="toolkit-delegate") as fake_delegate:
